            ...
            {recoverLeads.map((lead) => (
              <tr key={lead.id} className="hover:bg-gray-50">
                <td className="px-6 py-5">
                  <div className="font-medium text-gray-950">{lead.name}</div>
                ...
                <td className="px-6 py-5">
                  {lead.nextStep === "Re-engage" && (
                    <Badge label="Re-engage" variant="green" />
                  )}
                  {lead.nextStep === "Follow up" && (
                    <Badge label="Follow up" variant="yellow" />
                  )}
                  {lead.nextStep === "Retry with new angle" && (
                    <Badge label="Retry" variant="default" />
                  )}
                  {lead.nextStep === "Revive deal" && (
                    <Badge label="Revive" variant="green" />
                  )}
                  {lead.nextStep === "Re-qualify" && (
                    <Badge label="Re-qualify" variant="default" />
                  )}
                </td>
              </tr>
            ))}
